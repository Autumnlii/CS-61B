...
public class SList {
    ...
    public void squish() {
        ...
        while(node.next != null){
            if(node.item.equals(node.next.item)) {
                node.next = node.next.next; // skip the middle one because it's repeated;
            } else {
                node = node.next;
            }
        }
    ...
    public void twin() {
        ...
        size = size *2; // expand the SList size;

        SListNode node = head;
        if (head == null) {
            return;
        } else{ // if head is not null, then start copy itself
            for ( int i = 0; i < size/2; i++) {
                node.next = new SListNode(node.item, node.next); // copy itself to the next value
