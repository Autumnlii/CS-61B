...
public class Homework3 {
    ...
    public static void smoosh(int[] ints) {
        ...
        for (int i = 0; i < ints.length -1; i++) { // we can't go to the last index
            if (ints[i] == ints[i+1]) {
                copy[end] = -1;
                end --;
            } else {
        ...
        copy[end] = ints[ints.length-1]; // move the last item to the end of the copied array( before -1)
        for (int j = 0; j < ints.length ; j ++){
            ints[j] = copy[j]; // we should return the original array
        }
